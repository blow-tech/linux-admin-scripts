...
if [[ $EUID -ne 0 ]]; then
  ...
  echo "Some security checks require root privileges for complete results."
  echo "For full audit, please run with: sudo $0"
  echo
  read -p "Continue anyway? (y/n): " continue_nonroot
  if [[ "$continue_nonroot" != "y" && "$continue_nonroot" != "Y" ]]; then
    echo "Exiting. Run with sudo for full results."
    exit 1
  fi
